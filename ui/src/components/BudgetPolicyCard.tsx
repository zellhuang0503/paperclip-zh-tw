...
function windowLabel(windowKind: BudgetPolicySummary["windowKind"], t: (key: string) => string) {
  return windowKind === "lifetime" ? t("budget.lifetimeBudget") : t("budget.monthlyUtcBudget");
}
    ...
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? `${summary.utilizationPercent}% ${t("budget.ofLimit")}` : t("budget.noCapConfigured")}
        </div>
      </div>
      <div>
        ...
        <div className="mt-1 text-xs text-muted-foreground">
          {t("budget.softAlertAt")} {summary.warnPercent}%{summary.paused && summary.pauseReason ? ` · ${summary.pauseReason} ${t("budget.pause")}` : ""}
        </div>
      </div>
    </div>
  ) : (
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("budget.executionPausedProject")
          : t("budget.heartbeatsPausedScope")}
      </div>
    </div>
  ) : null;
